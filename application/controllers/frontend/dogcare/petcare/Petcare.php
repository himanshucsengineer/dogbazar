      ...
      foreach($data->result() as $row)
      {
        $output .= '<div class="card">
                        <div class="inner_card">
                            <video src="'.base_url().'upload/dogcare/petcare/'.$row->image.'" alt="" controls></video>
                            <h3 class="VIDEOHEEE"><a href="" class="newviesrc" data-bs-toggle="modal" data-video="'.base_url().'upload/dogcare/petcare/'.$row->image.'" data-id="'.$row->head.'" data-bs-target="#video">'.$row->head.'</h3>
                        </div>
                    </div>';
      }
     }
